 ...
 * (AP 9 of phase 2; the participant client's suite has the same file).
 *
 * A browser test that asserted `getByRole('button', { name: 'Save' })` asserts
 * two things at once — that the button is there, and that the English catalogue
 * says "Save". The second one is the organization's to change: a wording it
 * corrects in the language administration must not fail a test. So every
 * assertion here names a **key**, and the expected text is looked up.
 *
 ...
 * `'admin.common.save'` against a button labelled `admin.common.save`… which is
 * why {@link expectNoRawKeys} exists as well.
 *
 ...
 *
 * Not for the language administration: that page *shows* catalogue keys, one
 * per row, which is its whole function.
 */
...
/**
 * A key's text as a pattern, with every placeholder standing for anything.
 *
 * For the sentences whose parameters a browser test cannot reproduce exactly —
 * a formatted period, a zone abbreviation that depends on the reader's
 * language. The alternative is asserting an English fragment, which is what
 * naming keys is for.
 */
export function tPattern(
  key: string,
  params: Readonly<Record<string, string | number>> = {},
  locale = 'en',
): RegExp {
  const filled = t(key, params, locale);
